...
def parse_args():
    ...
    parser.add_argument('--type', type=str, default='m',
                        help='node type for classification. Default is movie.')
                        
    parser.add_argument('--alpha', type=float, default=0.5,
                        help='The value of alpha. Default is 0.5.')
    ...
    parser.add_argument('--output',type=str, default='0422xiuembedding.txt',
                        help='Input dataset path')
...
def generate_walks_new(G, alpha, type, type_att, vectors):
    walker = RWGraph(G)
    all, walks, paths = walker.simulate_walks_new(args.num_walks, args.walk_length, alpha, type, type_att, vectors)
...
if __name__ == "__main__":
    ...
    X, Y = read_node_label(file_name + '/label.txt')

    type_att = dict()
    for node in nodes:
        type_att[node] = [0.5,0.5]
    for epoch in range(args.epochs):
        print('epoch num:', epoch)
        if epoch < 1:
            all, walks, paths= generate_walks(G, args.alpha, type, type_att)
        else:
            all, walks, paths= generate_walks_new(G, args.alpha, type, type_att, vectors)
        pairs = generate_pairs(walks,paths)

        model = Word2Vec(all, vector_size=args.dimensions, window=args.window_size, min_count=0, negative = 5, epochs = 5, workers = 5, sg=1)
        vectors = model.wv
